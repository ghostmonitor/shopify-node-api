...
ShopifyAPI.prototype.buildAuthURL = function(getUserBasedToken = false) {
    var parsedUrl = url.parse(this.config.shop);
    // Since shop can be a slug and also an url let's handle if slug given
    // Also handle if it's an url but without protocol
    if (!parsedUrl.host) {
        // Let's check if it's an url without protocol
        if (this.config.shop.split('.').length - 1 === 0) {
            parsedUrl.host = this.config.shop + '.myshopify.com';
        } else {
            parsedUrl.host = this.config.shop;
        }
    }
    ...
    if (getUserBasedToken) {
      auth_url += `&grant_options[]=per-user`
    }
